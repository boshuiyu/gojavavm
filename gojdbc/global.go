...
func checkJVMSatus() error {
	...
	if errInitJavaVM != nil {
		errInitJavaVM = fmt.Errorf(`gojdbc.jar->%s, jvm动态库->%s, err->%v`, gJarFilePath, sopath, errInitJavaVM)
	}
	return errInitJavaVM
}

//全局参数，用
var gJarFilePath = "./gojdbc.jar"
var gDriverClass = ""
...
func loadJvmAndLibrary() error {
	var err error
	if err, sopath = pJDBCVM.InitJVM(gJarFilePath); err != nil {
		return err
	}
