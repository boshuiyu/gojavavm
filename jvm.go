...
func (this *JVM) InitJVM(jarFile string) (error, string) {
	if this.pJavaVM != nil {
		return nil, ""
	}
	this.jvmLib = this.QueryJVMLib()
	fmt.Println("查找jvm.dll，位置：", this.jvmLib)
	if this.jvmLib == "" {
	...
	if err != nil {
		this.UninitJVM()
		return err, this.jvmLib
	}
	return nil, this.jvmLib
}
...
func (this *JVM) getParamResult(v interface{}) (nType, nLength C.int, nValue C.jlong) {
	switch v.(type) {
	case []byte:
		...
		if bt, ok := v.([]byte); ok && len(bt) > 0 {
			...
			pBuffer := C.CBytes(bt)
			nValue = C.jlong(uintptr(pBuffer))
		}
