...
#else
		void * handle = dlopen(pszJvmLib, RTLD_NOW);
		if (handle == NULL){
			mErrMsg = FormatString("CreateJavaVM LoadLibrary Error:%s", strerror(errno));
			return false;
		}
		pfnJNI_CreateJavaVM = (PFN_JNI_CreateJavaVM)dlsym(handle, "JNI_CreateJavaVM");
		if(pfnJNI_CreateJavaVM == NULL){
			mErrMsg = "CreateJavaVM Lost Function:JNI_CreateJavaVM";
			return false;
		}
